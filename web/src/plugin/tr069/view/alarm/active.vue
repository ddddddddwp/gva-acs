  ...
  <div>
      ...
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item label="告警来源">
          <el-select v-model="searchInfo.source" placeholder="请选择" clearable>
            <el-option label="CurrentAlarm" value="CurrentAlarm" />
      ...
      <el-table :data="tableData" row-key="ID" stripe style="width: 100%">
        ...
        <el-table-column label="告警来源" prop="source" width="140">
          <template #default="scope">
            <el-tag :type="scope.row.source === 'CurrentAlarm' ? 'primary' : 'warning'">
              {{ scope.row.source }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="可能原因" prop="probableCause" min-width="150" show-overflow-tooltip />
    ...
    <el-dialog v-model="dialogVisible" title="告警详情" width="60%">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="最后变更">{{ formatDate(detailData.lastChanged) }}</el-descriptions-item>
      </el-descriptions>
    </el-dialog>
  </div>
</template>
...
<script setup>
import { ref } from 'vue'
import { getActiveAlarms } from '@/plugin/tr069/api/alarm'
...
const getTableData = async() => {
  const table = await getActiveAlarms({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
