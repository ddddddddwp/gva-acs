...
- 在 TR-069 监听端口 7458 增加 `PUT/POST /acs/log`，并为以后同样支持 PUT/POST 的 `/acs/pm`、`/acs/mr` 预留可注册文件通道；两种上传方法共用流式文件处理链，均不进入 CWMP XML 解析和 RawDump。
- 使用配置文件中的一套长期共享 LOG 用户名和密码校验所有基站上传，支持 HTTP Basic 和 Digest；账号或密码为空时通道不得启动，缺失或错误认证一律拒绝。
...
- 主动 Upload RPC 与周期上传都形成可审计的 LOG 任务；主动任务继续使用 CommandKey 关联 CWMP 状态，周期上传根据接收到的文件自动建档。
- 在 GVA 的 TR-069 菜单下新增“日志文件”页面，支持分页、按设备 ID 过滤和权限受控的日志下载。
...
- GVA 前端：TR-069 菜单、日志文件查询页、设备 ID 筛选、状态展示和下载操作。
...
- 部署：新增文件通道共享认证和 MinIO 配置，并要求为 7458 文件流量配置合适的代理超时与请求体限制。
